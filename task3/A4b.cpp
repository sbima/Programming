...
/*
* Decides whether arg date comes before first
* 
*
* @param first: first date
* @param arg: second date
* @return: true if arg date comes before first, false otherwise
*/
bool isArgumentDateBefore(Date first, Date arg)
{
	//case: 05:16:2000 vs 05:16:2001
	if( arg.year < first.year )
	{
		return true;
	}
	else if( arg.year > first.year )
	{
		return false;
	}
	...
	if( arg.year == first.year )
	{
		...
		if( arg.month < first.month )
		{
			return true;
		}
		else if( arg.month > first.month )
		{
			return false;
		}
...
Student* sortStudents(Student* students, int sizeOfStudents, string criteria)
{
	...
	for(int i = 0; i<sizeOfStudents; i++)
	{
		for(int j = i+1; j<sizeOfStudents; j++)
		{
			...
			{
				if( isArgumentDateBefore(students[j].graduationDate, students[i].graduationDate) )
				{
					shouldSwapFlag = true;
				}
			}
